
public class Avion extends Vehiculo {
	
	// atributos
		private boolean trenAterrizaje;
		
	// metodos
	public void volar() {}
	public void aterrizar() {}
	
	
	// constructor de la clase Avion
		public Avion(float velocidad, int asientos, int orientacionRuedas, boolean trenAterrizaje) {
			
			// invocacion al constructor de la superclase (Vehiculo)
			super(velocidad, asientos, orientacionRuedas);
			
			//System.out.println("Constructor de la clase Avion");		
			// inicializacion del atributo
			this.trenAterrizaje = trenAterrizaje;
		}
		
		// metodo acelerar de avion
		public void acelerar() {
			System.out.println("Acelerar en avion");
		}
		
		// metodos get y set para acceder desde fuera de la clase
		public boolean isTrenAterrizaje() {
			return trenAterrizaje;
		}
		
		// metodo set para modificarlos
		public void setTrenAterrizaje(boolean trenAterrizaje) {
			this.trenAterrizaje = trenAterrizaje;
		}
		
		// el metodo abstracto para de la clase Vehiculo se esta sobreescribiendo
		@Override
		public void parar() {
			System.out.println("Metodo parar de avion");
			
		}
		

}
